...
Bubbles = () ->
  ...
  updateActive = (id) ->
    if !(id=="") 
      lastId = id
    #awful one liner to get the label for this id
    myLabel = $($.grep(label[0],(n,i) ->  return n.href.split("#")[1]==lastId )[0]).children().first()
    ...
    # call read here
      myAjaxCall = $.ajax({
                 url: "http://localhost:8983/solr/kindle.highlights/select?q=*%3A*&fq=highlight%3A"+id+"&wt=json&indent=true",
        ...
        crossDomain: true,
        jsonp: 'json.wrf'
        }).done((data)-> read(data,myLabel,id) )
...
$ ->
  ...
  # ---
  display = (data) ->
    newRecord = {"name":searchTerm, "count":data.response.numFound}

    myData[myData.length] = newRecord


    plotData("#vis", myData, plot)
